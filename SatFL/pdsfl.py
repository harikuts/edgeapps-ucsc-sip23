...
Differs from existing decentralized SatFL methods (DSFL) in that:
- assumes full connectivity between clusters: for every pair of clusters their exists pair of connected sats from the corres. clusters
- Does not rely on 'multi-hop' propagation of cluster aggregates
    - multi-hop/path-finding: has higher chance of error : traveling across several inter-cluster-links (potentially volatile bc relative speed of satellites in diff clusters)
    - Pairwise: Just 'single-hop' communication btw pairs of clusters
- Works best for constellations where orbits have lots of physical intersection (ex Walker Delta)
...
if rank == server_rank:
    ...
    for it in range(num_iters+1):
        '''Inter-Orbit Link Generation: (only for non-physical simulation)'''
        # server generates randomized inter-cluster connections at the beginning of each iteration
        # these generated connections broadcasted to all satellites
        # (only in simulation is server involved each iteration, in reality connections would be defined by the physical positions of satellites but can't model this without satellite tool kit)
        cluster_conns = {c : [] for c in range(num_clusters)} # external sat connections for each cluster
        for c_1 in range(num_clusters):
            for c_2 in range(num_clusters):
                # for every pair of clusters, generate (random) connection
                if c_2 > c_1:
                    i = -1
                    j = -1
                    cluster_1_index = random.randint(0, len(cluster_ranks[c_1]) - 1)
                    i = cluster_ranks[c_1][cluster_1_index]
                    cluster_2_index = random.randint(0, len(cluster_ranks[c_2]) - 1)
                    j = cluster_ranks[c_2][cluster_2_index]
                    cluster_conns[c_1].append((i,j))
...
if rank != server_rank:
    ...
    if self_clust_ranks[0] == rank:  # 'first' client of the cluster (cluster rank of 0)
        ...
        model_info = comm.recv(source=left_cluster_neighbor,tag=1)  # model_info[0]: sender_rank, model_info[1]: initial model, model_info[2]: num of clients in cluster model has been propagated to
    ...
    for it in range(num_iters+1):
        cluster_conns = {c : [] for c in range(num_clusters)}
        cluster_conns = comm.bcast(obj=cluster_conns, root=server_rank)
        ...
        if local_rank != 0:
            external_clusters_aggregate = comm.recv(source=left_cluster_neighbor, tag=3)  # receives global external update
        ...
        # check if satellite responsible for an inter-cluster connection
        ext_clust_aggr = []
        for conn in cluster_conns[map_rank_to_cluster_id[rank]]:
            if rank == conn[0]:
                ...
                if conn[0] < conn[1]:
                    ...
                    ext_clust_aggr = comm.recv(source=conn[1], tag=6)
                else:
                    ext_clust_aggr = comm.recv(source=conn[1], tag=6)
                    comm.send(dest=conn[1],obj=copy.deepcopy(self_model.get_weights()),tag=6)
